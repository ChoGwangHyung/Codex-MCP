...
| Command | 목적 |
| --- | --- |
| `codex-telegram-permission-hook` | Telegram-origin turn의 Codex native `PermissionRequest` 승인을 처리합니다. |
| `codex-telegram-stop-hook` | Telegram-origin turn의 최종 응답을 Codex `Stop` hook에서 전송합니다. |
...
- native permission approval 또는 자동 relay 회신을 사용하려면 Codex hooks 활성화.
...
패키지는 `codex-telegram-bridge-mcp` MCP binary, `codex-telegram-configure`
설정 helper, Codex hook binary를 제공합니다.
...
대상 프로젝트 루트에서 setup helper로 local MCP와 hook 설정을 설치할 수 있습니다.

```powershell
codex-telegram-configure install-project
```

npm global 설치가 아니라 local repo checkout에서 실행할 때는 다음처럼 사용합니다.

```powershell
node <Codex-MCP>/codex-telegram-bridge-mcp/scripts/telegram-configure.js install-project
```

이 명령은 다음 파일을 생성하거나 갱신합니다.
...
env 파일 이름이 `.codex/config.toml.env`이면 bridge와 bundled hook은 기본적으로
chat allowlist는 `.codex/config.toml.access.json`, shared state는
`.codex/telegram-runtime`을 사용합니다. Telegram에서 받은 파일은 기본적으로
`.codex/telegram-runtime/downloads`에 저장됩니다.
`CODEX_TELEGRAM_BRIDGE_RUNTIME_DIR`, `CODEX_TELEGRAM_BRIDGE_DOWNLOAD_DIR`,
`CODEX_TELEGRAM_BRIDGE_ACCESS_FILE`은 custom path가 꼭 필요할 때만 설정하세요.
override할 경우 MCP server inline env에만 넣지 말고 env 파일에도 넣어야 합니다.
Codex hook process는 MCP server 전용 env 값을 상속하지 않습니다.

chat id는 보통 pairing helper나 `telegram-configure.js allow <chat-id>`가 관리하는
`.codex/config.toml.access.json`에만 둡니다. native permission approval도 기본적으로
같은 allowlist를 쓰므로 일반 설정에서는 `TELEGRAM_ALLOWED_CHAT_IDS`와
`CODEX_TELEGRAM_APPROVAL_CHAT_IDS`가 필요하지 않습니다.

`install-project`는 `.codex/.gitignore`도 작성해서 실제 secret, allowlist,
runtime download가 기본적으로 로컬에만 남도록 합니다. 커밋에는 안전한
`.codex/config.toml.env.example`만 포함합니다.
...
- Telegram-origin prompt에는 `telegram_send` 지시를 붙이지 않습니다. bundled
  `Stop` hook이 turn 완료 후 최종 assistant 메시지를 원래 Telegram chat으로
  전송합니다.
...
path가 있을 경우 해당 path만 포함합니다. 긴 Telegram file ID와 추가 파일
metadata는 console 전송 안정성을 위해 injected prompt에서는 제외하고, 전체
metadata는 inbox attachment 객체에 남깁니다. `app-server` mode에서는 다운로드된
image attachment를 native `localImage` 입력으로도 함께 전달합니다. Telegram
...
- approval prompt는 Telegram relay에서 시작된 요청을 처리하는 중일 때만
  실행됩니다. Telegram runtime state에 저장된 pending relay turn/session으로
  매칭합니다.
- Telegram-origin turn 안에서는 shell escalation, managed-network approval,
  `apply_patch`, MCP tool approval 같은 Codex `PermissionRequest` 이벤트를
  처리할 수 있습니다.
...
- MCP 서버가 Telegram 설정과 함께 시작되면 user-level hook을 기본으로 자동
  설치합니다. CLI-origin turn은 무시하고, Telegram-origin turn 중 발생한 native
  permission request만 Telegram으로 전달합니다.
- Codex는 새로 설치되거나 변경된 hook에 대해 실행 전 `/hooks` review를 요구할
  수 있습니다. bridge는 hook 설정을 설치하지만 사용자를 대신해 review 완료
  상태로 표시하지는 않습니다.
- bundled `PostToolUse` hook은 Telegram-origin permission request가 CLI prompt로
  fallback된 뒤 CLI에서 승인되어 tool이 실행되면 Telegram 메시지를 승인 처리
  상태로 업데이트합니다. CLI 거부는 tool이 실행되지 않기 때문에 현재 Codex hook
  이벤트만으로는 확정 감지할 수 없습니다.
